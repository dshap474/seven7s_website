...
import DataChartD3 from './components/DataChartD3';
import BacktestMetrics from './components/BacktestMetrics';
import BacktestChart from './components/BacktestChart';
import Intelligence from '@/components/Intelligence';
import { CHART_COLORS } from './components/DataChartD3';
...
const Analytics: React.FC = () => {
  ...
  const [selectedIndicator, setSelectedIndicator] = useState<string>('total_oi');
  const [selectedIndicators, setSelectedIndicators] = useState<string[]>([selectedIndicator]);
  ...
  // Create merged data from all selected indicators
  const getMergedData = () => {
    if (!data || selectedIndicators.length === 0) return [];
    
    // Use the first indicator's data as base for timestamps
    const baseData = data[selectedIndicators[0]];
    if (!baseData) return [];
    
    return baseData.map(basePoint => {
      const mergedPoint: any = { index: basePoint.index };
      
      // Add data from all selected indicators
      selectedIndicators.forEach(indicator => {
        const indicatorData = data[indicator];
        if (indicatorData) {
          const matchingPoint = indicatorData.find(p => p.index === basePoint.index);
          if (matchingPoint) {
            mergedPoint[indicator] = matchingPoint[indicator];
          }
        }
      });
      
      // Always include BTC price
      if (basePoint.btc_price) {
        mergedPoint.btc_price = basePoint.btc_price;
      }
  ...
  // Initialize selectedIndicators with the first indicator when data is loaded
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      const firstIndicator = Object.keys(data)[0];
      setSelectedIndicator(firstIndicator);
      setSelectedIndicators([firstIndicator]);
    }
  }, [data]);
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="text-white p-4 max-w-7xl mx-auto">
        <div className="bg-[#131722] p-4 rounded-lg h-[800px] border border-gray-800">
          <div className="h-full">
            <DataChartD3 
              data={getMergedData()} 
              title={selectedIndicator.replace(/_/g, ' ')} 
              series={Object.fromEntries([
                ...selectedIndicators.map(indicator => [
                  indicator,
                  {
                    label: indicator.replace(/_/g, ' ').toUpperCase(),
                    color: indicator === selectedIndicator ? CHART_COLORS.firstSeries : CHART_COLORS.secondSeries,
                    visible: true
                  }
                ]),
                ['btc_price', {
                  label: 'BTC Price',
                  color: CHART_COLORS.btcPrice,
                  visible: true
                }]
              ])}
              availableIndicators={indicators.filter(i => !selectedIndicators.includes(i))}
